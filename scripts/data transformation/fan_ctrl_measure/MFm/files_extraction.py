...
writer = pd.ExcelWriter('combined_MFm_8760s.xlsx', engine='xlsxwriter')

for cz in cz_folders:
    print(f"merging {cz} data")
    subpath = path + "/runs" + f"/{cz}/MFm&0&rDXGF&Ex&dxAC_equip/dxAC-Res-SEER-13.0/instance-var.csv"
...
write1 = pd.ExcelWriter('end_uses_combined_MFm.xlsx', engine='xlsxwriter')
write2 = pd.ExcelWriter('fan_table_combined_MFm.xlsx', engine='xlsxwriter')
write3 = pd.ExcelWriter('airloophvac_combined_MFm.xlsx', engine='xlsxwriter')

for cz in cz_folders:
    print(f"getting {cz} html outputs...")
    subpath = path + "/runs"+ f"/{cz}/MFm&0&rDXGF&Ex&dxAC_equip/dxAC-Res-SEER-13.0/instance-tbl.htm"
