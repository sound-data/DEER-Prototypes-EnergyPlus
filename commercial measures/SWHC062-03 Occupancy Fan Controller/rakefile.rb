...
file results_profile_elec_path => run_csv_paths do
  if File.exist?(results_profile_elec_path) && File.size(results_profile_elec_path).zero?
    File.delete(results_profile_elec_path)
  end
...
file results_profile_gas_path => run_csv_paths do
  if File.exist?(results_profile_gas_path) && File.size(results_profile_gas_path).zero?
    File.delete(results_profile_gas_path)
  end
...
def aggregate_profiles(column_name, output_path, run_csv_paths, runs_pathname, study_dir)
  ...
  valid_paths = []
  run_csv_paths.each do |run_csv_path|
    short_path = Pathname.new(run_csv_path).relative_path_from(runs_pathname)
    meter_csv_path = resolve_meter_csv_path(run_csv_path)

    if meter_csv_path && File.exist?(meter_csv_path)
      valid_paths << [short_path, meter_csv_path]
    else
      puts "warning: meter data file not found for #{short_path}\n"
    end
  end

  if valid_paths.empty?
    raise("No input CSV files found for #{output_path}")
  end

  detected_headers = valid_paths.map { |_, csv_path| detect_column_header(csv_path, column_name) }

  if detected_headers.all?(&:nil?)
    puts "warning: could not find column #{column_name.inspect} in any input CSV; writing zero values\n"
  end

  File.open(output_path, "w") do |file|
    file.puts(["Date/Time", *valid_paths.map { |short_path, _| short_path }].join(","))

    iterators = valid_paths.map { |_, csv_path| CSV.open(csv_path, "r", :headers=>true) }
    rows = []
    ...
    while rows.any? { |row| !row.nil? }
      first_row = rows.find { |row| !row.nil? }
      row_values = [first_row ? first_row["Date/Time"] : ""]
      rows.each_with_index do |row, index|
        if row && detected_headers[index]
          row_values << row[detected_headers[index]]
        else
          row_values << "0"
        end
      end
      ...
      rows = iterators.map.with_index do |iterator, index|
        next nil if rows[index].nil?
        begin
          iterator.shift
        rescue StopIteration
          nil
    ...
    iterators.each(&:close)
  end
end


def resolve_meter_csv_path(run_csv_path)
  case_dir = File.dirname(run_csv_path)
  candidates = Dir.glob("#{case_dir}/**/eplusmtr.csv")
  candidates.select { |path| File.file?(path) }.sort_by { |path| File.mtime(path) }.reverse.first
end


def detect_column_header(csv_path, column_name)
  csv = CSV.open(csv_path, "r", :headers=>true)
  row = csv.first
  csv.close

  if row
    matching_header(row.headers, column_name)
  else
    nil
  end
end
...
def matching_header(headers, column_name)
  target = normalize_header_value(column_name)
  headers.each do |header|
    normalized_header = normalize_header_value(header)
    return header if normalized_header == target || normalized_header.include?(target)
  end
...
def normalize_header_value(value)
  value.to_s.downcase.gsub(/\s+/, "").gsub(/[^a-z0-9]/, "")
end
