...
def search_directories(path, file_name):
    paths = []
    for dir_name, sub_dirs, files in Path(path).walk():
        for file in files:
...
def by_batch(offset, all_files):
    ...
    for file in all_files:
        ...
        cz = parts[offset + 1]
        if not re.fullmatch(r"CZ\d\d", cz):
            continue  # skip stale CZ##-old backup folders
...
def main():
    ...
    measure_dir = PurePath(
        r"C:\dev\SWHC062-03\commercial measures\SWHC062-03 Occupancy Fan Controller"
    )

    # Experiment subfolders to process. Each is expected to contain a "runs"
    # directory laid out as runs/CZ##/<BldgType>/<M#-cSYS-Measure>/instance-var.csv.
    study_folders = [
        "SWHC062-03 Occupancy Fan Controller_Htl_Ex",  # Hotel
        "SWHC062-03 Occupancy Fan Controller_Ex",      # all other building types
    ]

    # Output folder (cleared each run so re-runs don't append duplicates).
    results_folder = PurePath(measure_dir, "hourly_results")
    out_dir = Path(results_folder)
    out_dir.mkdir(parents=True, exist_ok=True)
    for old in list(out_dir.glob("*.csv")) + list(out_dir.glob("*-columns.txt")):
        old.unlink()

    # Results file_name
    results_file_name = "instance-var.csv"

    # With search paths of the form <measure_dir>/<study>/runs, the climate zone
    # (CZ##) lands at parts[offset + 1], building type at parts[offset + 2], and
    # the run folder at parts[offset + 3] -- matching by_batch()/set_up().
    offset = len(measure_dir.parts) + 1

    # Gather all hourly results files across every study folder.
    all_files = []
    for study in study_folders:
        search_path = Path(measure_dir, study, "runs")
        if not search_path.is_dir():
            print("Skipping (no runs yet): {}".format(search_path))
            continue
        found = search_directories(search_path, results_file_name)
        print("Found {} '{}' files in {}".format(len(found), results_file_name, study))
        all_files.extend(found)

    if not all_files:
        print("No results files found. Have the simulations been run?")
        return
