...
# 2024-01-26 Afshin Faramarzi 
# Added tout_ave and delta_t_max
# This customization was developed as a workaround to resolve an EnergyPlus
# issue impacting water mains temperature in residential models (EnergyPlus v9.5).
...
if (not rake_task_name =~ /^(clean|none)$/)
  ...
  # 2024-01-26 Afshin Faramarzi 
  # Customization here developed as a workaround to resolve an EnergyPlus
  # issue impacting water mains temperature in residential models (EnergyPlus v9.5).
  # Revised in 2025 to move lookup data to a separate file.
  water_params_given_cz = water_params[climate_name]
  puts "water_params for given climate: #{water_params_given_cz}"
	tout_ave = water_params_given_cz[:tout_ave]
	delta_t_max = water_params_given_cz[:delta_t_max]
  # End customization
