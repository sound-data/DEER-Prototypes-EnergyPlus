...
file results_profile_gas_path => run_csv_paths do
  aggregate_profiles("NaturalGas:Facility", results_profile_gas_path, run_csv_paths, runs_pathname, study_dir)
end
...
def aggregate_profiles(column_name, output_path, run_csv_paths, runs_pathname, study_dir)
  ...
  short_paths = run_csv_paths.map do |path|
    Pathname.new(path).relative_path_from(runs_pathname).to_s
  end
  ...
  # ---------------------------------------------------------
  # OPEN ALL STREAMS (NO FULL FILE LOAD)
  # ---------------------------------------------------------
  short_paths.each do |short_path|
    csv_path = File.join(runs_pathname, short_path)

    if File.exist?(csv_path)
      stream = CSV.open(csv_path, "r", headers: true)

      # SAFE HEADER ACCESS (NO &.)
      first_row = stream.first
      headers = first_row ? first_row.headers : nil
      stream.rewind

      if column_header.nil? && headers
        column_header = headers.find { |h| h.match(column_name) }
      end
  ...
  # ---------------------------------------------------------
  # STREAM OUTPUT (ROW-BY-ROW, NO TRANSPOSE)
  # ---------------------------------------------------------
  CSV.open(output_path, "w") do |out|

    # write header once
    out << ["Date/Time"] + short_paths
    ...
    loop do
      ...
      streams.each do |stream|
        ...
        time_value ||= row["Date/Time"]
        row_values << row[column_header]
        active_streams += 1
      end

      # stop when all streams are exhausted
      break if active_streams == 0

      out << [time_value] + row_values
    end
  end
